...
（仅客户端）发送给 `viewerUrl` 和 `textUrl` 的额外 HTTP 请求头。可用于设置类似浏览器的请求头，例如 `Accept-Language` 或 `Referer`。 (v5.51.2+)

如果 `requestHeaders` 包含 `User-Agent`，它会覆盖 `userAgent`。如果包含 `Host`，它会作为 HTTP Host 头处理并覆盖 `viewerHostHeader`。这些请求头只用于客户端到 Google Docs Viewer 的请求；它们不会控制 Google Docs Viewer 访问 `originUrl` 时发送的请求头。
...
* 直接使用 `gdocsviewer` 传输时会使用带有空闲轮询退避和服务端长轮询的 request assembler。空轮询请求可能会等待一段时间后才返回，以避免空闲连接持续消耗 Google Docs Viewer 配额。
